...
[cols=",",]
|===
|xref:module-graph-browser:graph-browser.adoc[Graph Browser] |Display
Elasticsearch documents as nodes and Siren Investigate relations as
links of a graph. *Note*: The Graph Browser is a very powerful visualization and is described in a separate section of the documentation.
...
Table:: A representation of the underlying data, presented as a paginated data
...
Statistics:: A summary of the statistics related to the request and the response,
...
URL format can be used to create a dynamic URL. The following image shows
...
Clicking *Export* in the dialog will begin exporting all the
documents matching a query on an Elasticsearch index in the format you
have chosen.

For more control over what gets exported, click *Additional
settings* to list some more advanced options.

By default, all fields of an index will be exported. To limit the export
to a specific set of fields, click the arrow next to 'Fields'. This will
...
(this is done automatically by the Label input field above), which is
