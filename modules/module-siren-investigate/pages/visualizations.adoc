...
delimiter[, limit])] |`+payload.IP BAR split('.', 3)+` |Splits an IP
...
substring[, length])] |`+payload.name BAR endsWith('smith', 10)+` |Returns
...
substring[, position])] |`+payload.name BAR startsWith('smith', 10)+`
...
substring[, length])] |`+payload.name BAR indexOf('smith', 10)+` |Returns
...
substring, newSubString)] |`+payload.name BAR replace('smith', 'jones')+`
